 ...
 * - Bridge dependencies
 */
export class ComposabilityAnalyzer {
  
  // Known protocol categories and their typical dependencies
  private readonly protocolDependencies: Record<string, string[]> = {
   ...
   */
  private analyzeDependencyDepth(protocol: ProtocolData): Finding[] {
    const findings: Finding[] = [];
   ...
   */
  private analyzeOracleConcentration(protocol: ProtocolData): Finding[] {
    const findings: Finding[] = [];
   ...
   */
  private analyzeLiquidityFragmentation(protocol: ProtocolData): Finding[] {
    const findings: Finding[] = [];
   ...
   */
  private analyzeCollateralChain(protocol: ProtocolData): Finding[] {
    const findings: Finding[] = [];
   ...
   */
  private analyzeReentrancyEconomics(protocol: ProtocolData): Finding[] {
    const findings: Finding[] = [];
   ...
   */
  private analyzeBridgeRisk(protocol: ProtocolData): Finding[] {
    const findings: Finding[] = [];
    ...
    const usesWrappedAssets = protocol.pools?.some(p => 
      p.token0.toLowerCase().includes('w') || 
