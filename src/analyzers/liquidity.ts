...
export class LiquidityAnalyzer {
  ...
  private analyzePoolLiquidity(pool: PoolData): { score: number; findings: Finding[] } {
    const findings: Finding[] = [];
    let score = 0;
  ...
  private analyzeImpermanentLoss(pools: PoolData[]): { score: number; findings: Finding[] } {
    const findings: Finding[] = [];
    let score = 0;

    // Check for volatile pair exposure
    const volatilePairs = pools.filter(p => 
      !p.token0.includes('USD') && !p.token1.includes('USD') &&
